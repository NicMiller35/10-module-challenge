        ...
        switch (action) {
            ...
            case 'Delete an employee':
                app.delete ('api/employee/:id', async (req, res) => {
                    const result: QueryResult = await pool.query('DELETE FROM employees WHERE id = $1', [req.params.id]);
                    res.json(result.rows);
                });
                break;
            case 'Delete a role':
                app.delete ('api/role/:id', async (req, res) => {
                    const result: QueryResult = await pool.query('DELETE FROM roles WHERE id = $1', [req.params.id]);
                    res.json(result.rows);
            });
                break;
            case 'Delete a department':
                app.delete ('api/department/:id', async (req, res) => {
                    const result: QueryResult = await pool.query('DELETE FROM departments WHERE id = $1', [req.params.id]);
                    res.json(result.rows);
                });
                break;
            case 'Update an employee role':
                    app.put ('api/employee/:id', async (req, res) => {
                        const result: QueryResult = await pool.query('UPDATE employees SET role_id = $1 WHERE id = $2', [req.body.role_id, req.params.id]);
                        res.json(result.rows);
                    });
                break;
        }
