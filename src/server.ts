...
const viewAllRoles = async () => {
    try {
        const result: QueryResult = await pool.query('SELECT roles.id AS "ID", roles.title AS "Job Title", roles.salary AS "Salary", departments.name AS "Department" FROM roles LEFT JOIN departments ON roles.department_id = departments.id');
        const table = new Table({
...
const viewAllDepartments = async () => {
    try {
        const result: QueryResult = await pool.query('SELECT id AS "ID", name AS "Department" FROM departments');
        const table = new Table({
            head: ['ID', 'Department'],
            colWidths: [5, 15],
        });
